...
type MockResponse = {
	statusCode: number;
	_listeners: Map<string, MockCallback>;
...
function createResponse(): MockResponse {
	const listeners = new Map<string, MockCallback>();
	const res = { statusCode: 200, _listeners: listeners } as MockResponse;
	...
	res.on = vi.fn().mockImplementation((event: string, cb: MockCallback) => {
...
describe("apiKeyValidation middleware", () => {
	...
	describe("validateApiKey", () => {
		it("returns 401 when API key is missing", async () => {
			...
			await validateApiKey(
				req as unknown as Parameters<typeof validateApiKey>[0],
				res as unknown as Parameters<typeof validateApiKey>[1],
				next,
			);
	...
	describe("enforceTierLimits", () => {
		it("calls next when api key is missing", () => {
			...
			enforceTierLimits(
				req as unknown as Parameters<typeof enforceTierLimits>[0],
				res as unknown as Parameters<typeof enforceTierLimits>[1],
				next,
			);
