...
const mockedPubFindMany = prisma.pub.findMany as unknown as ReturnType<
	typeof vi.fn
>;
const mockedAuditLogCount = prisma.auditLog.count as unknown as ReturnType<
	typeof vi.fn
>;
const mockedAuditLogFindMany = prisma.auditLog
	.findMany as unknown as ReturnType<typeof vi.fn>;
const mockedTransaction = prisma.$transaction as unknown as ReturnType<
	typeof vi.fn
>;
const mockedBatchCheckRateLimits =
	batchCheckRateLimits as unknown as ReturnType<typeof vi.fn>;
...
describe("POST /auth/forgot-api-key", () => {
	...
	it("returns generic success when account is not found to prevent email enumeration", async () => {
		...
		expect(response.body).toEqual({
			message:
				"If the email exists, a new API key has been generated and sent to your email.",
		});
		expect(mockedUserFindUnique).toHaveBeenCalledWith({
			where: { email: "missing@example.com" },
...
describe("GET /auth/dashboard", () => {
	...
	it("returns dashboard data with limits and summary", async () => {
		...
		mockedBatchCheckRateLimits.mockResolvedValueOnce(
			new Map([
				[
					"api_key_1",
					{
	...
	it("falls back to zero in summary when usageCount is missing", async () => {
		...
		mockedBatchCheckRateLimits.mockResolvedValue(
			new Map([
				[
					"api_key_a",
					{
...
describe("categorizeEditTypes", () => {
	it("categorizes address fields", () => {
		expect(
			categorizeEditTypes({ address: "1 New St", postcode: "E1 1AA" }),
		).toEqual(["address"]);
	});

	it("categorizes feature fields", () => {
		expect(
			categorizeEditTypes({ hasFood: true, isDogFriendly: false }),
		).toEqual(["features"]);
	});

	it("categorizes detail fields", () => {
		expect(
			categorizeEditTypes({ name: "New Name", website: "https://example.com" }),
		).toEqual(["details"]);
	});
	...
	it("categorizes opening hours", () => {
		expect(categorizeEditTypes({ openingHours: {} })).toEqual([
			"opening hours",
		]);
	});

	it("returns multiple categories for mixed changes", () => {
		const result = categorizeEditTypes({
			address: "1 New St",
			hasFood: true,
			beerTypes: [],
		});
		expect(result).toEqual(
			expect.arrayContaining(["address", "features", "beer types"]),
		);
...
describe("GET /auth/contributions", () => {
	...
	it("returns totalAdded, recentPubs, totalEdited and recentEdits for an authenticated user", async () => {
		...
		const editedPubs = [
			{ id: "pub_2", name: "The Edited Pub", city: "Manchester" },
		];

		mockedPubCount.mockResolvedValueOnce(1);
		mockedPubFindMany
			.mockResolvedValueOnce(recentPubs)
			.mockResolvedValueOnce(editedPubs);
		...
		expect(response.body.recentEdits[0].editTypes).toEqual(
			expect.arrayContaining(["address", "features"]),
		);
	});
