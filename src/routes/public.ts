...
router.get(
	...
	async (req: ApiKeyRequest, res: Response) => {
		try {
			...
			const { remaining, resetTimes } =
				rateLimitResult ?? (await checkRateLimit(id, tier));
...
router.get(
	...
	async (req: ApiKeyRequest, res: Response) => {
		try {
			...
			const sinceDate =
				since && !Number.isNaN(Date.parse(since as string))
...
router.get("/info", async (_req: ApiKeyRequest, res: Response) => {
	res.json({
		success: true,
		api: {
		...
		endpoints: {
			...
			"GET /api/v1/usage/history":
				"Get your recent API request history (supports ?limit, ?since, ?endpoint)",
			"GET /api/v1/info": "Get API information",
		},
		usage: {
			authentication: "API key required for all endpoints except /info",
			apiKey: "Include API key in 'X-API-Key' header",
			tiers: {
				TESTING:
