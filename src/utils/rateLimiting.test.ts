...
describe("rateLimiting utils", () => {
	...
	describe("checkRateLimit", () => {
		it("denies access when api key is missing", async () => {
			...
			const result = await checkRateLimit("missing", "HOBBY" as ApiKeyTier);
		...
		it("calculates remaining limits and allows request when under limits", async () => {
			...
			const result = await checkRateLimit("key_1", "HOBBY" as ApiKeyTier);
			...
			expect(result.resetTimes.hour.toISOString()).toBe(
				"2026-03-06T11:00:00.000Z",
			);
			expect(result.resetTimes.day.toISOString()).toBe(
				"2026-03-07T00:00:00.000Z",
			);
			expect(result.resetTimes.month.toISOString()).toBe(
		...
		it("resets monthly usage when reset date has passed", async () => {
			...
			await checkRateLimit("key_2", "DEVELOPER" as ApiKeyTier);
		...
		it("denies request when hourly limit is exhausted", async () => {
			...
			const result = await checkRateLimit("key_3", "HOBBY" as ApiKeyTier);
	...
	describe("batchCheckRateLimits", () => {
		...
		it("computes remaining limits from usage counts", async () => {
			mockApiKeyUsageGroupBy
				...
				.mockResolvedValueOnce([
					{ apiKeyId: "key_1", _count: { apiKeyId: 15 } },
				]); // daily
		...
		it("denies when hourly limit is exhausted", async () => {
			mockApiKeyUsageGroupBy
				.mockResolvedValueOnce([
					{ apiKeyId: "key_2", _count: { apiKeyId: 20 } },
				]) // hourly maxed
				.mockResolvedValueOnce([]);
